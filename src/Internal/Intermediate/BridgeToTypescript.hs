...
bConstructedToTS :: (IsForeignType (TSCustom f), IsForeignType (TSComposite f)) => (BType -> Maybe (TSType f)) -> Text -> BConstructor -> Maybe (TSType f)
bConstructedToTS bridgeTypeToTSType typeName bcon =
  ...
  where
  handleSingle :: (IsForeignType (TSCustom f), IsForeignType (TSComposite f)) => BSingleConstructorArg -> Maybe (TSField f)
  handleSingle c =
    case c of
      OfRecord f   -> bfieldToTSField f
      OfUnTagged _ -> Nothing

bfieldToTSField :: (IsForeignType (TSCustom f), IsForeignType (TSComposite f)) => (BField) -> Maybe (TSField f)
